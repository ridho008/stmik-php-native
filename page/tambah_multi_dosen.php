...
if(isset($_POST['tambah_multi_dosen'])) {
  if(multi_tambah_dosen($_POST) > 0) {
    echo "<script>alert('Data Dosen Berhasil Ditambahkan.');window.location='dosen.php';</script>";
  } else {
    echo "<script>alert('Data Dosen Gagal Ditambahkan.');window.location='dosen.php';</script>";
  }
}
...
<head>
	...
	<title>Tambah Multi Dosen</title>
  ...
  <form action="" method="post" enctype="multipart/form-data">
    ...
    <table class="table">
      ...
      <tr>
        ...
        <td>
          <input type="number" name="nidn-<?= $i ?>" class="form-control" required>
        </td>
        <td>
          <input type="text" name="keahlian-<?= $i ?>" class="form-control" required>
        </td>
        <td>
          <input type="radio" name="jk-<?= $i ?>" value="L"> L
          <input type="radio" name="jk-<?= $i ?>" value="P"> P
        </td>
      </tr>
    <?php endfor; ?>
    </table>
    <div class="form-group">
      <button type="submit" name="tambah_multi_dosen" class="btn btn-primary">Tambah Data</button>
    </div>
  </form>
